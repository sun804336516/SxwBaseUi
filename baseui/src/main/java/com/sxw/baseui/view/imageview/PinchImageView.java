...
public class PinchImageView extends ImageView {
    ...
    public Matrix getInnerMatrix(Matrix matrix) {
        ...
        if (isReady()) {
            ...
            int pWid, pHei;
            pWid = getDrawable().getIntrinsicWidth();
            pHei = getDrawable().getIntrinsicHeight();

            int widthPixels = metrics.widthPixels;
            int heightPixels = metrics.heightPixels;
            //原图大小
            RectF tempSrc = MathUtils.rectFTake(0, 0, pWid, pHei);
            //控件大小
            RectF tempDst = MathUtils.rectFTake(0, 0, getWidth(), getHeight());
            //会出现中心偏移情况
//            RectF tempDst = MathUtils.rectFTake(0, 0, widthPixels, heightPixels);
//            LogUtils.LogE(tempSrc + "============================================" + tempDst);
            //计算fit center矩阵
            ...
            if (vW > 1.0f) {
                scale = heightPixels * 1.0f / pHei;
                scale = scale / floats[0];
                HORIZONTAL_SCAL = scale;
//                matrix.postScale(scale, scale, 0, heightPixels >> 1);
            }
            if (vH >= 4.0f) {
                scale = widthPixels * 1.0f / pWid;
                scale = scale / floats[0];
                matrix.postScale(scale, scale, widthPixels >> 1, 0);
            }
        }
        return matrix;
    }
    ...
    protected float calculateNextScale(float innerScale, float outerScale) {
        ...
        if (currentScale < MAX_SCALE) {
            if (HORIZONTAL_SCAL > 1.0f) {
                return currentScale * HORIZONTAL_SCAL;
            } else {
                return MAX_SCALE;
            }
        } else {
...
//        }
        ...
        RectF testBound = MathUtils.rectFTake(0, 0
