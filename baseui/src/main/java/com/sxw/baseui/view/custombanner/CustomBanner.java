...
public class CustomBanner<T> extends FrameLayout {
    ...
    private Runnable mTurningTask = new Runnable() {
        @Override
        public void run() {
            Log.d("onWindowFocusChanged", "run:" + isTurning + "--" + mBannerViewPager);
    ...
    private void getAttrs(Context context, AttributeSet attrs) {
        if (attrs != null) {
            ...
            mIndicatorInterval = mTypedArray.getDimensionPixelOffset(
                    R.styleable.custom_banner_indicatorInterval, ViewUtils.dp2px(5));
...
//        drawable.setIntrinsicWidth(interval);
    ...
    public void onWindowFocusChanged(boolean hasWindowFocus) {
        ...
        Log.d("onWindowFocusChanged", "" + hasWindowFocus);
        if (isTurning) {
            if (hasWindowFocus) {
    ...
    protected void onAttachedToWindow() {
        ...
        Log.d("onWindowFocusChanged", "onAttachedToWindow");
//        startTurning(mIntervalTime);
    }
...
//        stopTurning();
        Log.d("onWindowFocusChanged", "onDetachedFromWindow");
