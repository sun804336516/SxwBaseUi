 ...
 */
package com.sxw.baseui.view.calendarview;
...
public final class CalendarUtil {
    ...
    public static int getMonthEndDiff(Calendar calendar, int weekStart) {
        java.util.Calendar date = java.util.Calendar.getInstance();
    ...
    public static List<Calendar> getWeekCalendars(Calendar calendar, CalendarViewDelegate mDelegate) {
        ...
        int week = date.get(java.util.Calendar.DAY_OF_WEEK);
        int startDiff;
        if (mDelegate.getWeekStart() == CalendarViewDelegate.WEEK_START_WITH_SUN) {
            startDiff = week - 1;
        } else if (mDelegate.getWeekStart() == CalendarViewDelegate.WEEK_START_WITH_MON) {
...
//        }
//        return mItems;
        return initCalendarForWeekView(calendar, mDelegate.getCurrentDay(), weekStart);
    }
     ...
     *
     * @param calendar  calendar
     * @param currenDay currenDay
     * @param weekStart weekStart
     * @return 生成周视图的7个item
     */
    public static List<Calendar> initCalendarForWeekView(Calendar calendar, Calendar currenDay, int weekStart) {
        ...
        selectCalendar.setMonth(date.get(java.util.Calendar.MONTH) + 1);
        selectCalendar.setDay(date.get(java.util.Calendar.DAY_OF_MONTH));
        if (selectCalendar.equals(currenDay)) {
            selectCalendar.setCurrentDay(true);
        }
        LunarCalendar.setupLunarCalendar(selectCalendar);
        selectCalendar.setCurrentMonth(true);
        ...
        for (int i = 1; i <= weekEndDiff; i++) {
            ...
            calendarDate.setMonth(date.get(java.util.Calendar.MONTH) + 1);
            calendarDate.setDay(date.get(java.util.Calendar.DAY_OF_MONTH));
            if (calendarDate.equals(currenDay)) {
                calendarDate.setCurrentDay(true);
            }
            LunarCalendar.setupLunarCalendar(calendarDate);
            calendarDate.setCurrentMonth(true);
     ...
     */
    @Deprecated
    public Calendar getCalendarFromMonthViewPager(int position, CalendarViewDelegate delegate) {
        Calendar calendar = new Calendar();
