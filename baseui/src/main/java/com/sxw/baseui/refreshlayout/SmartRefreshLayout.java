package com.sxw.baseui.refreshlayout;
...
import com.sxw.baseui.refreshlayout.api.DefaultRefreshFooterCreator;
import com.sxw.baseui.refreshlayout.api.DefaultRefreshHeaderCreator;
import com.sxw.baseui.refreshlayout.api.DefaultRefreshInitializer;
import com.sxw.baseui.refreshlayout.api.RefreshContent;
import com.sxw.baseui.refreshlayout.api.RefreshFooter;
import com.sxw.baseui.refreshlayout.api.RefreshHeader;
import com.sxw.baseui.refreshlayout.api.RefreshInternal;
import com.sxw.baseui.refreshlayout.api.RefreshKernel;
import com.sxw.baseui.refreshlayout.api.RefreshLayout;
import com.sxw.baseui.refreshlayout.api.ScrollBoundaryDecider;
import com.sxw.baseui.refreshlayout.constant.DimensionStatus;
import com.sxw.baseui.refreshlayout.constant.RefreshState;
import com.sxw.baseui.refreshlayout.constant.SpinnerStyle;
import com.sxw.baseui.refreshlayout.footer.BallPulseFooter;
import com.sxw.baseui.refreshlayout.header.BezierRadarHeader;
import com.sxw.baseui.refreshlayout.impl.RefreshContentWrapper;
import com.sxw.baseui.refreshlayout.impl.RefreshFooterWrapper;
import com.sxw.baseui.refreshlayout.impl.RefreshHeaderWrapper;
import com.sxw.baseui.refreshlayout.listener.OnLoadMoreListener;
import com.sxw.baseui.refreshlayout.listener.OnMultiPurposeListener;
import com.sxw.baseui.refreshlayout.listener.OnRefreshListener;
import com.sxw.baseui.refreshlayout.listener.OnRefreshLoadMoreListener;
import com.sxw.baseui.refreshlayout.listener.OnStateChangedListener;
import com.sxw.baseui.refreshlayout.util.DelayedRunnable;
import com.sxw.baseui.refreshlayout.util.SmartUtil;
import com.sxw.baseui.refreshlayout.util.ViscousFluidInterpolator;
...
public class SmartRefreshLayout extends ViewGroup implements RefreshLayout, NestedScrollingParent/*, NestedScrollingChild*/ {
    ...
    protected float mHeaderMaxDragRate = 3f;  //最大拖动比率(最大高度/Header高度)
    ...
    private static final int aniTime = 1500;
    ...
    public SmartRefreshLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mFooterHeight = ViewUtils.dp2px(60);
        mHeaderHeight = ViewUtils.dp2px(100);
...
//                mRefreshContent = new RefreshContentWrapper(view);
    ...
    protected void onAttachedToWindow() {
        ...
        if (!thisView.isInEditMode()) {
            ...
            if (mRefreshContent == null) {
                final int padding = ViewUtils.dp2px(20);
        ...
        public void run() {
            if (animationRunnable == this && !mState.isFinishing) {
                ...
                if (Math.abs(velocity) >= 1) {
                    ...
                    animationRunnable = null;
                    if (Math.abs(mSpinner) >= Math.abs(mSmoothDistance)) {
                        int duration = 10 * Math.min(Math.max((int) ViewUtils.px2dp(Math.abs(mSpinner - mSmoothDistance)), 30), 100);
...
//    }
    ...
    public SmartRefreshLayout setFooterHeight(float heightDp) {
        if (mFooterHeightStatus.canReplaceWith(DimensionStatus.CodeExact)) {
            mFooterHeight = ViewUtils.dp2px(heightDp);
    ...
    public SmartRefreshLayout setHeaderHeight(float heightDp) {
        if (mHeaderHeightStatus.canReplaceWith(DimensionStatus.CodeExact)) {
            mHeaderHeight = ViewUtils.dp2px(heightDp);
    ...
    public SmartRefreshLayout setHeaderInsetStart(float insetDp) {
        mHeaderInsetStart = ViewUtils.dp2px(insetDp);
        return this;
    }
    ...
    public SmartRefreshLayout setFooterInsetStart(float insetDp) {
        mFooterInsetStart = ViewUtils.dp2px(insetDp);
        return this;
    }
...
//    }
     ...
     * Recommended {@link com.sxw.baseui.refreshlayout.listener.SimpleMultiPurposeListener}
     * 设置多功能监听器
     * 建议使用 {@link com.sxw.baseui.refreshlayout.listener.SimpleMultiPurposeListener}
     *
     ...
     * Recommended {@link com.sxw.baseui.refreshlayout.impl.ScrollBoundaryDeciderAdapter}
     * 设置滚动边界判断器
     * 建议使用 {@link com.sxw.baseui.refreshlayout.impl.ScrollBoundaryDeciderAdapter}
     *
    ...
    public SmartRefreshLayout finishRefresh() {
        ...
        return finishRefresh(Math.min(Math.max(0, aniTime - (int) passTime), aniTime));//保证刷新动画有aniTime毫秒的时间
    }
    ...
    public SmartRefreshLayout finishLoadMore() {
        long passTime = System.currentTimeMillis() - mLastOpenTime;
        return finishLoadMore(Math.min(Math.max(0, aniTime - (int) passTime), aniTime));//保证加载动画有300毫秒的时间
    }
    ...
    public SmartRefreshLayout finishRefresh(boolean success) {
        long passTime = System.currentTimeMillis() - mLastOpenTime;
        return finishRefresh(success ? Math.min(Math.max(0, aniTime - (int) passTime), aniTime) : 0, success);//保证加载动画有aniTime毫秒的时间
    }
